...
class CompressByTuckerVideoRecognizer(nn.Module):
    def __init__(self, input_shape, tucker_ranks, n_classes):
        ...
        self.concat_fc1 = nn.Linear(n_classes * 10, n_classes * 5)
        self.concat_fc2 = nn.Linear(n_classes * 5, n_classes)

        self.relu = nn.ReLU()
        self.pool = nn.AvgPool1d(3)

        self.hankel_matrix_conv2d_1 = nn.Conv2d(1, 4, 3, 2, 0)
        self.hankel_matrix_conv2d_2 = nn.Conv2d(4, 4, 3, 2, 0)
        self.hankel_matrix_conv2d_3 = nn.Conv2d(4, 8, 3, 2, 0)
        self.hankel_matrix_conv2d_4 = nn.Conv2d(8, 8, 3, 2, 0)
        self.hankel_matrix_conv2d_5 = nn.Conv2d(8, 16, 3, 2, 0)
        self.hankel_matrix_fc_1 = nn.Linear(1024, 512)
        self.hankel_matrix_fc_2 = nn.Linear(512, 256)
        self.hankel_matrix_fc_3 = nn.Linear(256, 120)
        self.hankel_matrix_bn_1 = nn.BatchNorm2d(4)
        self.hankel_matrix_bn_2 = nn.BatchNorm2d(4)
        self.hankel_matrix_bn_3 = nn.BatchNorm2d(8)
        self.hankel_matrix_bn_4 = nn.BatchNorm2d(8)
        self.hankel_matrix_bn_5 = nn.BatchNorm2d(16)
    ...
    def forward_hankel_matrix(self, x):
        x = x.unsqueeze(0).transpose(1, 0)
        x = self.relu(self.hankel_matrix_bn_1(self.hankel_matrix_conv2d_1(x)))
        x = self.relu(self.hankel_matrix_bn_2(self.hankel_matrix_conv2d_2(x)))
        x = self.pool2d(x)
        x = self.relu(self.hankel_matrix_bn_3(self.hankel_matrix_conv2d_3(x)))
        x = self.relu(self.hankel_matrix_bn_4(self.hankel_matrix_conv2d_4(x)))
        x = self.pool2d(x)
        x = self.relu(self.hankel_matrix_bn_5(self.hankel_matrix_conv2d_5(x)))
        x = nn.Flatten()(x)
        x = self.relu(self.hankel_matrix_fc_1(x))
        x = self.relu(self.hankel_matrix_fc_2(x))
        x = self.hankel_matrix_fc_3(x)

        return x

    def forward(self, data):
        cores, factor_matrices_1, factor_matrices_2, factor_matrices_3, factor_matrices_4, hankel_matrices = data
        out_core_tensor = self.forward_core_tensor(cores)
        out_factor_matrix_1 = self.forward_factor_matrix_1(factor_matrices_1)
        out_factor_matrix_2 = self.forward_factor_matrix_2(factor_matrices_2)
        out_factor_matrix_3 = self.forward_factor_matrix_3(factor_matrices_3)
        out_factor_matrix_4 = self.forward_factor_matrix_4(factor_matrices_4)
        out_hankel_matrix = self.forward_hankel_matrix(hankel_matrices)

        concat_ft = torch.cat(
            [out_core_tensor, out_factor_matrix_1, out_factor_matrix_2, out_factor_matrix_3, out_factor_matrix_4, out_hankel_matrix],
            dim=1)
        out = self.relu(self.concat_fc1(concat_ft))
        out = self.concat_fc2(out)
